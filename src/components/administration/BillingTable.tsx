        ...
        <div className="bg-white rounded-[32px] border border-zinc-200 shadow-sm overflow-hidden mb-12 animate-fade-in">
            ...
            <div className="h-[600px]">
                <TableVirtuoso
                    ...
                    itemContent={(_, row) => {
                        const clubCommValue = row.clubCommissionType === 'fixed' ? (row.clubCommissionFixed || 0) : ((row.salary || 0) * (row.clubCommissionPct || 0) / 100);
                        const playerCommValue = row.playerCommissionType === 'fixed' ? (row.playerCommissionFixed || 0) : ((row.salary || 0) * (row.playerCommissionPct || 0) / 100);
                        const totalComm = clubCommValue + playerCommValue;
                        const hasClubComm = (row.clubCommissionType === 'fixed' ? (row.clubCommissionFixed || 0) : (row.clubCommissionPct || 0)) > 0;
                        const hasPlayerComm = (row.playerCommissionType === 'fixed' ? (row.playerCommissionFixed || 0) : (row.playerCommissionPct || 0)) > 0;
                        const currencySymbol = row.currency === 'USD' ? '$' : row.currency === 'GBP' ? '£' : '€';
                            ...
                            <>
                                ...
                                <td className="px-6 py-4 border-b border-zinc-100 bg-inherit">
                                    ...
                                    <div className="mt-2 pl-12 text-[10px] text-zinc-400 font-bold">
                                        Comisión: {totalComm.toLocaleString('es-ES', { style: 'currency', currency: row.currency || 'EUR', maximumFractionDigits: 0 })}
                                    </div>
                                </td>
                                    ...
                                    {hasClubComm ? (
                                        <div className="flex flex-col items-center gap-2 bg-zinc-50/50 p-2 rounded-xl border border-zinc-100/50">
                                            <div className="text-[10px] text-zinc-400 font-bold mb-1">
                                                {row.clubCommissionType === 'fixed'
                                                    ? `${(row.clubCommissionFixed || 0).toLocaleString('es-ES')} ${currencySymbol} Club`
                                                    : `${row.clubCommissionPct || 0}% Club`}
                                            </div>
                                        ...
                                        <div className="h-full flex flex-col items-center justify-center opacity-30">
                                            <span className="text-zinc-400 text-xs italic font-bold">
                                                0{row.clubCommissionType === 'fixed' ? ` ${currencySymbol}` : '%'} Club
                                            </span>
                                ...
                                <td className="px-6 py-4 align-top border-b border-zinc-100 bg-inherit">
                                    {hasPlayerComm ? (
                                        <div className="flex flex-col items-center gap-2 bg-zinc-50/50 p-2 rounded-xl border border-zinc-100/50">
                                            <div className="text-[10px] text-zinc-400 font-bold mb-1">
                                                {row.playerCommissionType === 'fixed'
                                                    ? `${(row.playerCommissionFixed || 0).toLocaleString('es-ES')} ${currencySymbol} Jugador`
                                                    : `${row.playerCommissionPct || 0}% Jugador`}
                                            </div>
                                            <div className="flex items-center gap-2 mb-1 w-full">
                                                <span className="text-[9px] font-black text-zinc-400 uppercase w-12 text-right">Estado</span>
                                                <select
                                        ...
                                        <div className="h-full flex flex-col items-center justify-center opacity-30">
                                            <span className="text-zinc-400 text-xs italic font-bold">
                                                0{row.playerCommissionType === 'fixed' ? ` ${currencySymbol}` : '%'} Jugador
                                            </span>
                                            <span className="text-[10px] text-zinc-300">Sin gestión</span>
                                        </div>
                                    )}
